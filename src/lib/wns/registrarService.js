import { getProvider, getWnsRegistrarAddress } from './web3Service';
...
const setWeb3Provider = () => {
  ...
  registrar = new Registrar(web3, getWnsRegistrarAddress(process.env.BLOCKCHAIN_NETWORK));
}
...
export const getShaBid = async (name, address, ether, secret) => {
  try {
    setWeb3Provider();
    return await registrar.shaBid(web3.sha3(name), address, web3.toWei(ether), web3.sha3(secret));
  } catch (err) {
